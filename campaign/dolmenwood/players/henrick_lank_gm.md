---
tags: [character, pc, human, hunter, level-1]
name: Henrick Lank
kindred: Human
class: Hunter
level: 1
alignment: Neutral
status: alive
created: 2026-02-19
---

# Henrick Lank
**Kindred:** Human (Mortal, Medium)
**Class:** Hunter · Level 1
**Alignment:** Neutral
**Background:** Soldier
**Languages:** Woldish

---

## Ability Scores

| STR | INT | WIS | DEX | CON | CHA |
|-----|-----|-----|-----|-----|-----|
| 5 (-2) | 13 (+1) | 10 (+0) | 10 (+0) | 14 (+1) | 9 (+0) |

**Prime Ability:** Constitution and Dexterity (14, 10) → **XP Modifier: +10%**

---

## Combat

| Stat | Value |
|------|-------|
| **HP** | 4 / 4 |
| **AC (Ascending)** | 12 |
| **AC (Descending)** | -2 |
| **Attack Bonus** | +1 |
| **Speed** | 40' (120' per Turn) |

**Saving Throws**

| Doom | Ray | Hold | Blast | Spell |
|------|-----|------|-------|-------|
| 12 | 13 | 14 | 15 | 16 |

> Roll d20 equal or above target to succeed. Wisdom modifier applies as Magic Resistance bonus to Spell saves.

---

## Armour & Weapons

| Item | Details |
|------|---------|
| Armour | Leather armour |
| Weapon 1 | Longbow + 20 arrows |
| Weapon 2 | Dagger |


**Gold:** 60 gp

---

## Class Abilities

**Combat Aptitude:** Martial
**Armour:** Light, shields
**Weapons:** Any

- Animal Companion: May bond with an animal. Companion follows hunter, fights to the death.
- Hunter Skills: Alertness, Stalking, Survival, Tracking.
- Missile Attack Bonus: +1 Attack with all missile weapons.
- Trophies: Take trophy from a hunted creature → +1 Attack vs same creature type.


### Skills

| Skill | Target | Roll |
|-------|--------|------|
| Alertness | 6 | d6, equal or beat target |
| Stalking | 6 | d6, equal or beat target |
| Survival | 5 | d6, equal or beat target |
| Tracking | 5 | d6, equal or beat target |

---

## Kindred Traits (Human)

- Decisiveness: When Initiative is tied, humans act first.
- Leadership: Retainer Loyalty +1.
- Spirited: +10% bonus to all XP earned.

---

## Personality

**Appearance:** *(fill in)*

**Demeanour:** *(fill in)*

**Wants:** *(fill in)*

**Fears:** *(fill in)*

---

## GM Notes *(remove for player handout)*

> **Secret:**
>
> **Personal hook:**
>
> **Loyalty – what would they betray the party for:**
