---
tags: [character, pc, woodgrue, enchanter, level-1]
name: Obblehob Woodfuffle
kindred: Woodgrue
class: Enchanter
level: 1
alignment: Chaotic
status: alive
created: 2026-02-19
---

# Obblehob Woodfuffle
**Kindred:** Woodgrue (Demi-fey, Small)
**Class:** Enchanter · Level 1
**Alignment:** Chaotic
**Background:** Fireworks maker
**Languages:** Woldish, Sylvan

---

## Ability Scores

| STR | INT | WIS | DEX | CON | CHA |
|-----|-----|-----|-----|-----|-----|
| 8 (-1) | 8 (-1) | 13 (+1) | 14 (+1) | 11 (+0) | 15 (+1) |

**Prime Ability:** Charisma and Intelligence (15, 8) → **XP Modifier: -10%**

---

## Combat

| Stat | Value |
|------|-------|
| **HP** | 3 / 3 |
| **AC (Ascending)** | 10 |
| **AC (Descending)** | 0 |
| **Attack Bonus** | +0 |
| **Speed** | 40' (120' per Turn) |

**Saving Throws**

| Doom | Ray | Hold | Blast | Spell |
|------|-----|------|-------|-------|
| 11 | 12 | 13 | 16 | 14 |

> Roll d20 equal or above target to succeed. Wisdom modifier applies as Magic Resistance bonus to Spell saves.

---

## Armour & Weapons

| Item | Details |
|------|---------|
| Weapon 1 | Dagger |
| Weapon 2 | Spear |


**Gold:** 110 gp

---

## Class Abilities

**Combat Aptitude:** Semi-martial
**Armour:** Light and Medium, no shields
**Weapons:** Small and Medium

- Fairy Runes: Knows 1 random rune of lesser magnitude at Lvl 1.
- Glamours: Knows 1 glamour at Lvl 1 (plus kindred glamours if any).
- Detect Magic Skill: Touch object/place/creature to detect magic.
- Resistance to Divine Aid: 2-in-6 chance beneficial holy spells have no effect.
- Kindred: Typically fairies and demi-fey. Rare mortals with Fairy connection also qualify.


### Skills

| Skill | Target | Roll |
|-------|--------|------|
| Detect Magic | 5 | d6, equal or beat target |

---

## Kindred Traits (Woodgrue)

- Enchanted Songs: Can use music to lure folk into woods and enchant them to frolic. Mortals within earshot must Save vs Spell or be fascinated.
- Remnant Glamours: Knows 1 random glamour (diminished fairy magic).
- Vulnerable to Cold Iron: Cold iron weapons deal +1 damage.
- Long-lived: Lifespan 300+ years but not immortal.

---

## Personality

**Appearance:** *(fill in)*

**Demeanour:** *(fill in)*

**Wants:** *(fill in)*

**Fears:** *(fill in)*
